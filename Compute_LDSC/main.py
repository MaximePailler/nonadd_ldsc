...
#-----------------------------------------------------------------------------------------------------------------------
if type_file == 1:
	(gen_A, pos, chrom, MAF) = Read_table.read_tped(path, min_maf, type_pos)
...
#----------------------------------------------------------------------------------------------------------------------
	...
	(gen_A, pos, chrom) = Read_table.read_bed(path, type_pos)
	print('data ok')
...
resid = Computation.calc_D_resid(gen_A, gen_D)
print('D_resid ok')
...
result = Computation.calc_LD_Score(gen_A, resid, pos, wind, MAF, chrom)
print('Results obtained')

#Output the results to a .csv file
result.to_csv(path_or_buf = path + "_output.csv", header = ["Chrom", "L_aa_orig", "L_ad_orig", "L_aa", "L_ad", "L_aa_new", "L_ad_new", "L_ad_new_adjust", "Nsnp","Nsnp_used", "MAF"], index = True, index_label = "SNP", sep = " ", na_rep = "Nan")
