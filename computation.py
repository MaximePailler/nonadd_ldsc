...
class Computation:
	...
	def calc_D_resid(gen_A, gen_D, sample):
		'''
		...
		'''
		resid = pd.DataFrame(np.nan, index = gen_A.index.values, columns = sample)
		i=1
		for snp in gen_A.index.values:
			...
			residual = pd.Series(residual.flatten(), index = sample)
			resid.loc[snp] = residual.values
		return resid



	def calc_LD_Score(gen_A, resid, pos, wind, MAF):
		'''
		...
		result = pd.concat([LD_Score, Nsnp_serie, Nsnp_used_serie, MAF], axis = 1)
